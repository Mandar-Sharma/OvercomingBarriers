...
model = DistilBertForMaskedLM.from_pretrained('textattack/distilbert-base-uncased-CoLA')
...
for i in tqdm(train_dataset):
    ...
    if len(sqr_gradients) == 100:
        for i in range(len(sqr_gradients[0])):
            ...
            sum_sqr_gradients.append(torch.mul(torch.sum(torch.stack(items),0), 1/8551))
        ...
        sum_sqr_gradients = []
    if len(ssg) == 100:
        with open('./Gradients/DistilBERT/CoLA/ssg_{}.pkl'.format(file_idx), 'wb') as fp:
            pickle.dump(ssg, fp)
        del ssg
        ssg = []
        file_idx += 1
...
files = glob('./Gradients/DistilBERT/CoLA/ssg_*')
fischer_1 = []
ssgs = []
...
with open('./Gradients/DistilBERT/CoLA/fisher_cola.pkl','wb') as fp:
